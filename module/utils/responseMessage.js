module.exports = {
  DB_ERROR: "DB 오류",
  NULL_VALUE: "필요한 값이 없습니다",
  OUT_OF_VALUE: "파라미터 값이 잘못되었습니다",

  FAIL_SIGNUP: "회원가입 실패",
  SUCCESS_SIGNUP: "회원가입 성공",
  ALREADY_EXIST_EMAIL: "이미 존재하는 이메일입니다.",
  USABLE_EMAIL: "사용가능한 이메일입니다.",

  NOT_EXIST_EMAIL: "존재하지 않는 이메일입니다.",

  SUCCESS_LOGIN: "로그인 성공",
  FAIL_LOGIN: "로그인 실패",
  NOT_CORRECT_PASSWORD: "비밀번호가 일치하지 않습니다.",

  SUCCESS_UPLOAD_GOODS: "경매상품 등록 성공",
  FAIL_UPLOAD_GOODS: "경매상품 등록 실패",

  SUCCESS_SELECT_GOODS_LIST: "경매상품 리스트 조회 성공",
  FAIL_SELECT_GOODS_LIST: "경매상품 리스트 조회 실패",

  SUCCESS_ENTER_THE_ROOM: "경매방 입장 성공",
  FAIL_ENTER_THE_ROOM: "경매방 입장 실패",

  BE_FINISHED_THE_AUCTION: "해당 경매가 이미 종료되었습니다",
  BE_HIGHER_THAN_PREVIOUS_BID: "이전 입찰가보다 높아야 합니다",
  BE_HIGHER_THAN_STARTING_PRICE: "시작 가격보다 높게 입찰해야 합니다.",
  SUCCESS_BIDDING: "입찰 성공",
  FAIL_BIDDING: "입찰 실패",

  SUCCESS_SCRAP: "향수 스크랩 성공",
  SUCCESS_DELETE_SCRAP: "스크랩한 향수 삭제 성공",
  FAIL_CHANGE_SCRAP_STATE: "향수 스크랩 상태 변경 실패",

  SUCCESS_SELECT_SCRAP: "스크랩한 향수 조회 성공",
  FAIL_SELECT_SCRAP: "스크랩한 향수 조회 실패",
  NOT_EXIST_SCRAP: "스크랩한 향수가 존재하지 않습니다",

  NO_TOKEN_VALUE: "토큰 값이 없습니다",
  EXPIRED_TOKEN: "유효하지 않은 토큰입니다",
  INVALID_TOKEN: "잘못된 토큰입니다",
  VALID_TOKEN: "유효한 토큰입니다",

  SUCCESS_SEARCH_PERFUME_LIST: "향수 리스트 검색 성공",
  FAIL_SEARCH_PERFUME_LIST: "향수 리스트 검색 실패",

  SUCCESS_RECOMMEND_PERFUME_NEW: "새로운 향수 추천 성공",
  FAIL_RECOMMEND_PERFUME_NEW: "새로운 향수 추천 실패",

  SUCCESS_RECOMMEND_PERFUME_BASED: "기존 향수기반 추천 성공",
  FAIL_RECOMMEND_PERFUME_BASED: "기존 향수기반 추천 실패",

  SUCCESS_SELECT_PERFUME_DATA: "향수 데이터 조회 성공",
  FAIL_SELECT_PERFUME_DATA: "향수 데이터 조회 실패",
};
